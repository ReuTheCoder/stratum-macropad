...
bool combo_key_was_combo[3] = {false, false, false};
    ...
    switch (settings_state) {
        case SSTATE_NORMAL:
            if (combo_keys_all_held()) {
                ...
                combo_key_was_combo[0] = true;  
                combo_key_was_combo[1] = true;   
                combo_key_was_combo[2] = true;  
                apply_rgb_for_state();
            }
            break;
...
void oled_write_centered_row(uint8_t row, const char *text) {
    char buf[22];
    uint8_t len = strlen(text);
    if (len > 21) len = 21;
    uint8_t col = (21 - len) / 2;

    memset(buf, ' ', 21);
    buf[21] = '\0';
    memcpy(buf + col, text, len);
...
void draw_layer_screen(void) {
    ...
    oled_write_centered_row(0, "");
    oled_write_centered_row(1, name); //CLEARS ROW
    oled_write_centered_row(2, "");
    oled_write_centered_row(3, "");
}

void draw_hold_progress_screen(void) {
    oled_write_centered_row(0, "Hold for settings");
    ...
    oled_write_centered_row(2, bar);
}

void draw_main_menu_screen(void) {
    oled_write_centered_row(0, "Settings");

    bool blink_on = (timer_read32() % 1000) < 500;
    char line[22];
    snprintf(line, sizeof(line), "%s%s", blink_on ? ">" : " ", main_menu_items[main_menu_index]);
    oled_write_centered_row(2, line);
}

void draw_layer_menu_screen(void) {
    oled_write_centered_row(0, "Layer");

    bool blink_on = (timer_read32() % 1000) < 500;
    const char *item = (layer_menu_index < 5) ? layer_name(layer_menu_index) : "Cancel";
    char line[22];
    snprintf(line, sizeof(line), "%s%s", blink_on ? ">" : " ", item);
    oled_write_centered_row(2, line);
}

void draw_brightness_menu_screen(void) {
    oled_write_centered_row(0, "Brightness");

    bool blink_on = (timer_read32() % 1000) < 500;
    uint8_t pct = brightness_menu_index * 10;
    char line[22];
    snprintf(line, sizeof(line), "%s%3d%%", blink_on ? ">" : " ", pct);
    oled_write_centered_row(2, line);
}

void draw_sleep_menu_screen(void) {
    oled_write_centered_row(0, "Sleep");
    ...
    if (sleep_menu_index <= 5) {
        if (sleep_timeout_minutes[sleep_menu_index] == 0) {
            ...
            snprintf(item, sizeof(item), "%d min", sleep_timeout_minutes[sleep_menu_index]);
        }
    } else if (sleep_menu_index == 6) {
        strcpy(item, "Sleep Now");
    } else {
        strcpy(item, "Cancel");
    }
